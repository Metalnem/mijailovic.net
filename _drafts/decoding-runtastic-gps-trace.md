...
As you might remember, one part of the activity data returned from the server was the GPS trace field, which I then ignored because there
...
the application to backup my latest runs, but I suddenly started getting errors that some expected headers in HTTP responses were missing.
...
request every 10 seconds. Such long delay was unacceptable to me. For example, if you run 5 times per week, and you were using Runtastic
for the last 4 years, you will have around 1000 activities, and your backup process would be almost 3 hours long. Web application was no
exception to the rate limiting - if you tried to export multiple activities in a small period of time, all files except the first one would
contain only "access denied" string. At that moment I had no other choice but to return to encoded GPS trace field, decode it and
...
I did that by exporting one of my activities and removing all except the first three GPS data points. The final gpx file that I decided
...
This looked promising. I feared that GPS trace might be encrypted, but this string had a lot of redundancy in it (enrypted data would
be indistinguishable from random data). Most of it were just alphanumeric characters - the rest were +, / and =. That suggested
[Base64](https://en.wikipedia.org/wiki/Base64) encoding. After decoding the string I got array of bytes looking like this:
...
in [big-endian](https://en.wikipedia.org/wiki/Endianness) format, followed by the actual encoding of every data point. After I removed
the first 4 bytes (length of one 32-bit integer), I tried to split the remaining bytes into three equal parts (and also convert them
...
This looked nice! Every row started with the same 6 bytes, and bunch of bytes on other positions were same in all rows, which confirmed
...
(or simply float and double). I didn't know which one the application was using, so I tried both. Converting longitude from the first
...
for converting time found in gpx file to unix time. First time was converted to 1473436853, which is 0x57d2dcb5 in hex. But that value
...
I couldn't conclude anything from those values, so I decided to look at the bigger picture. I downloaded one of my larger runs and
...
That didn't look like one 32-bit integer to me. It was more probable there were actually two 16-bit integers there. When converted to
...
run was 71m, and elevation loss was 66m!
...
Following the same procedure I used for decoding the GPS trace, I found values 148, 152 and 147 at the position 8. The time was still
